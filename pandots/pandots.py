...
def draw_ggplot(pairs,xlab,ylab,tlab,klab,key):
    df = pd.DataFrame([(x1, y1, x2, y2, genome) for (x1, y1), (x2, y2), genome in pairs], columns=['xstart', 'ystart', 'xstop', 'ystop', 'genome'])
    if key:
...
def main():
    ...
    plot = draw_ggplot(pairs,args.ref,'','','Genome',args.key)
